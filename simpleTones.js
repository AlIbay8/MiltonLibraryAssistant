...
function playTone(frequency, type, duration) {
	...
	o = contextA.createOscillator();
	g = contextA.createGain();
	o.connect(g);
	o.type = type;
	...
	g.connect(contextA.destination);
	o.start(0);
	g.gain.exponentialRampToValueAtTime(0.0001,contextA.currentTime + duration);
}

//This function helps complete chords and should not be used by itself
function completeChord(frequency, type, duration) {
	var osc = contextA.createOscillator();
	var gn = contextA.createGain();
	osc.connect(gn);
	osc.type = type;
	osc.frequency.value = frequency;
	gn.connect(contextA.destination);
	osc.start(0);
	gn.gain.exponentialRampToValueAtTime(0.0001,contextA.currentTime + duration);
}
...
//This function plays sounds
  function playSound(waveType,startFreq,endTime) {
	...
	var oscillatorNode = contextA.createOscillator();
	var gainNode = contextA.createGain();
	
	oscillatorNode.type = waveType;
	oscillatorNode.frequency.setValueAtTime(startFreq, contextA.currentTime);
	
for (var i = 3; i < arguments.length; i += 2) {
	oscillatorNode.frequency.exponentialRampToValueAtTime(arguments[i], contextA.currentTime + arguments[i+1]);
}
	gainNode.gain.setValueAtTime(0.3, contextA.currentTime);
	gainNode.gain.exponentialRampToValueAtTime(0.1, contextA.currentTime +  0.5);
  
	oscillatorNode.connect(gainNode);
	gainNode.connect(contextA.destination);
  
	oscillatorNode.start();
	oscillatorNode.stop(contextA.currentTime + endTime);
  }
}
